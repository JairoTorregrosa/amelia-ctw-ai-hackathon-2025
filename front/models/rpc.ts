...
export class RpcModel {
   ...
   */
  async createActiveInsights(args: Database['public']['Functions']['create_active_insights']['Args']) {
   ...
   */
  async getConversationTimeoutMinutes(args: Database['public']['Functions']['get_conversation_timeout_minutes']['Args']) {
   ...
   */
  async setConversationTimeoutMinutes(args: Database['public']['Functions']['set_conversation_timeout_minutes']['Args']) {
   ...
   */
  async closeInactiveConversations(args: Database['public']['Functions']['close_inactive_conversations']['Args']) {
    const { data, error } = await supabase.rpc('close_inactive_conversations', args);
    return ensure<Database['public']['Functions']['close_inactive_conversations']['Returns']>(data, error);
  }

  /**
   * Get completed insights for a conversation
   */
  async getCompletedInsightsForConversation(args: Database['public']['Functions']['get_completed_insights_for_conversation']['Args']) {
    const { data, error } = await supabase.rpc('get_completed_insights_for_conversation', args);
    return ensure<Database['public']['Functions']['get_completed_insights_for_conversation']['Returns']>(data, error);
  }

  /**
   * Complete multiple insights in batch
   */
  async completeInsightBatch(args: Database['public']['Functions']['complete_insight_batch']['Args']) {
    const { data, error } = await supabase.rpc('complete_insight_batch', args);
    return ensure<Database['public']['Functions']['complete_insight_batch']['Returns']>(data, error);
  }
   ...
   */
  async getOldestPendingInsights(args: Database['public']['Functions']['get_oldest_pending_insights']['Args']) {
    const { data, error } = await supabase.rpc('get_oldest_pending_insights', args);
    return ensure<Database['public']['Functions']['get_oldest_pending_insights']['Returns']>(data, error);
  }

  /**
   * Get patient pending insights
   */
  async getPatientPendingInsights(args: Database['public']['Functions']['get_patient_pending_insights']['Args']) {
    const { data, error } = await supabase.rpc('get_patient_pending_insights', args);
    return ensure<Database['public']['Functions']['get_patient_pending_insights']['Returns']>(data, error);
  }

  /**
   * Get scheduled jobs status
   */
  async getScheduledJobsStatus(args: Database['public']['Functions']['get_scheduled_jobs_status']['Args']) {
    const { data, error } = await supabase.rpc('get_scheduled_jobs_status', args);
    return ensure<Database['public']['Functions']['get_scheduled_jobs_status']['Returns']>(data, error);
  }

  /**
   * Run conversation cleanup now
   */
  async runConversationCleanupNow(args: Database['public']['Functions']['run_conversation_cleanup_now']['Args']) {
    const { data, error } = await supabase.rpc('run_conversation_cleanup_now', args);
    return ensure<Database['public']['Functions']['run_conversation_cleanup_now']['Returns']>(data, error);
  }

  /**
   * Cleanup orphaned insights
   */
  async cleanupOrphanedInsights(args: Database['public']['Functions']['cleanup_orphaned_insights']['Args']) {
    const { data, error } = await supabase.rpc('cleanup_orphaned_insights', args);
    return ensure<Database['public']['Functions']['cleanup_orphaned_insights']['Returns']>(data, error);
  }

  /**
   * Pause conversation cleanup job
   */
  async pauseConversationCleanupJob(args: Database['public']['Functions']['pause_conversation_cleanup_job']['Args']) {
    const { data, error } = await supabase.rpc('pause_conversation_cleanup_job', args);
    return ensure<Database['public']['Functions']['pause_conversation_cleanup_job']['Returns']>(data, error);
  }

  /**
   * Resume conversation cleanup job
   */
  async resumeConversationCleanupJob(args: Database['public']['Functions']['resume_conversation_cleanup_job']['Args']) {
    const { data, error } = await supabase.rpc('resume_conversation_cleanup_job', args);
    return ensure<Database['public']['Functions']['resume_conversation_cleanup_job']['Returns']>(data, error);
  }
}
